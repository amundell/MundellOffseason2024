...
public class SwerveSubsystem extends SubsystemBase
{
  ...
  public Rotation2d getSpeakerYaw()
  {
    ...
    return new Rotation2d(relativeTrl.getX(), relativeTrl.getY()).plus(swerveDrive.getOdometryHeading()).plus(Rotation2d.fromDegrees(180));
  }
   ...
   */
  public Command aimAtSpeaker(DoubleSupplier vx, DoubleSupplier vy, double tolerance)
  {
    SwerveController controller = swerveDrive.getSwerveController();
    return run(
        () -> {
          drive(ChassisSpeeds.fromFieldRelativeSpeeds(vy.getAsDouble()*swerveDrive.getMaximumVelocity(),
                                                      vx.getAsDouble()*swerveDrive.getMaximumVelocity(),
...
// Since AutoBuilder is configured, we can use it to build pathfinding commands
  ...
  private void updatePose3d()
  {
    boolean doRejectUpdate = false;
      ...
      if(Math.abs(swerveDrive.getRobotVelocity().omegaRadiansPerSecond) > Units.degreesToRadians(720)) // if our angular velocity is greater than 720 degrees per second, ignore vision updates
      {
        doRejectUpdate = true;
      }
      if(mt2.tagCount == 0)
